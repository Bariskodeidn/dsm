      ...
      <?php if ($menus): ?>
        <?php foreach ($menus as $menu):
          ...
          <?php if (empty($menu->submenus)): ?>
            <li class="nav-item w-100">
              <a class="nav-link <?= ($controller == $menu->controller) ? 'active' : '' ?>"
                href="<?= site_url($menu_url) ?>">
                <i class="<?= $menu->icon ?>"></i>
            ...
            <li class="nav-item <?= ($controller == $menu->controller) ? 'active' : '' ?> dropdown">
              ...
              <ul class="collapse <?= ($controller == $menu->controller) ? 'show' : '' ?> list-unstyled pl-4 w-100" id="<?= $menu->url ?>">
                <?php foreach ($menu->submenus as $submenu):
                  ...
                  <li class="nav-item">
                    <a class="nav-link <?= ($current_uri == $submenu->url) ? 'active' : '' ?> pl-3"
                      href="<?= site_url($submenu_url) ?>">
                      <i class="<?= $submenu->icon ?>"></i>
                      <span class="ml-1 item-text"><?= $submenu->menu_name ?>
                      </span>
                    </a>
                  </li>
                <?php endforeach; ?>
